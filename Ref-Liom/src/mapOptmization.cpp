...
public:
    ...
    mapOptimization()
    {
        ...
        pubKeyPoses   = nh.advertise<sensor_msgs::PointCloud2>("/ref_liom/key_poses", 10);//关键帧位姿点云
        pubGlobalMap  = nh.advertise<sensor_msgs::PointCloud2>("/ref_liom/global_map", 10);//发布优化后的全局地图
        pubAftPgoOdom = nh.advertise<nav_msgs::Odometry> ("/ref_liom/aftPgo_Odom", 10);//发布激光里程计
        pubAftPgoPath = nh.advertise<nav_msgs::Path>("/ref_liom/aftPgo_Path", 10);//发布优化后的全局轨迹
        pubSubmap =     nh.advertise<sensor_msgs::PointCloud2>("/ref_liom/submap", 10);//发布子图点云
        pubModelPoses = nh.advertise<nav_msgs::Path>("/ref_liom/model_poses", 10);//先验模型优化位姿
    ...
    */
    ...
    void syncedCallback(const nav_msgs::Odometry::ConstPtr &msgOdom, const sensor_msgs::PointCloud2::ConstPtr &msgCloud)
    {
        ...
        latestOdomPose = poseMsg2gtsamPose( msgOdom->pose.pose );//最新里程计位姿
    ...
    */
    void performLoopClosure()
    {
        ...
        {//加锁
            ...
            sigLoopConstraint.wait(ulock, []{return !holdForLoop;});
            holdForLoop = true;
        }
    ...
    */
    ...
    bool saveFrame()
    {
        ...
        Eigen::Vector3d rotBetween = latestOdomPose.rotation().localCoordinates(keyPoses6D.back().pose.rotation());
        gtsam::Point3 transBetween = latestOdomPose.translation() - keyPoses6D.back().pose.translation();
    ...
    void addRefModelFactor(){
        while (!refQueue.empty()){
            ...
            gtsam::Pose3 poseBetween = poseMsg2gtsamPose(refMsg.pose);
            int model_idx = static_cast<int>(refMsg.covariance[0]);//模型id
            uint32_t keypose_idx = static_cast<uint32_t>(refMsg.covariance[1]);//关键帧id
            double var_trans = refMsg.covariance[2];//位移协方差
            double var_rot = refMsg.covariance[3];//旋转协方差
            noiseModel::Diagonal::shared_ptr refNoise = noiseModel::Diagonal::Variances((Vector(6) << var_trans, var_trans, var_trans, var_rot, var_rot, var_rot).finished());
            gtSAMgraph.add(BetweenFactor<Pose3>(X(keypose_idx), M(model_idx), poseBetween, refNoise));//加入模型因子
            gtsam::Pose3 poseFrom = keyPoses6D[keypose_idx].pose;//关键帧位姿
            initialEstimate.insert(M(model_idx), poseFrom * poseBetween);//加入初值
            refQueue.pop_front();
            model_indices.emplace(model_idx);//将模型id加入集合model_indices中
        }
        aLoopIsClosed = true;
    }
    /**
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        // if(subframe_idx < submap_frame_num){
            ...
            // for(auto i : hash_submap[submap_idx]->keyCloudIndices){
                ...
                if(i >= keyClouds.size()) cout<<"error"<<endl;
                if(keyClouds[i]->points.size()==0) cout<<"error2"<<endl;
                //将关键帧点云都投影到最后一帧的坐标系中
    ...
    void correctPoses()
    {
        ...
        if (aLoopIsClosed == true)//一旦有新的Prior Factor, GPS Factor 或 Loop Factor加入，则更新历史关键帧位姿
        {
            ...
            for(auto idx : model_indices){
                ...
                pose_stamped.pose = gtsamPose2poseMsg( isamCurrentEstimate.at<Pose3>(M(idx)) );
